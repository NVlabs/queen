...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, latent_args: QuantizeParams, model_args: ModelParams, frame_idx : int = 1):
    ...
    def setup_decoders(self, latent_args: QuantizeParams, verbose=False):
        ...
        for i, param_name in enumerate(self.param_names):
            ...
            if verbose:
                print(f"DEBUG(): GaussianModel::setup_decoders(): param_name: {param_name}, created decoder in quant_type: {latent_args.quant_type[i]}, type: {type(self.latent_decoders[param_name])}")
    ...
    @property
    def _xyz(self):
        # first pass the xyz latents through the latent decoder to get the xyz attribute
        # then, if the attributes are gated and we have a previous frame's attributes,
        # we use the gate attributes and the previous frame's values to generate the xyz
        # values after gating, and return the result.
        # else, we just return the xyz attribute after latent decoding.
    ...
    def compress_gated_attribute(self, attribute_name):
        """
        Compresses a gated attribute by storing:
        1. The gating pattern (both mask and actual gate values)
        2. The residual values for active gates only
        3. The previous attribute values
        """

        # need to save the arrows perfectly to map between the prev_atts and the G_3_expanded
        # case 1 (split),  we need to save where they are copied from and copied to
        # case 2 (clone), we need to save where they are copied from and copied to
        # case 3 (prune): some elements got deleted from G_3, need to be ignored 

        # goal: G3_expended - G3[expand_idx]

        # Get the full attribute and its residual
        full_attr = getattr(self, f"_{attribute_name}")
        prev_attr = self.prev_atts[attribute_name]
        
        # Calculate the residual
        residual = full_attr - prev_attr
        
        # Get the gating pattern
        gate_pattern = self.gate_atts.get_gating_pattern()
        
        # Get the active values from the residual
        active_mask = torch.from_numpy(gate_pattern['active_mask']).to(residual.device)
        active_gate_values = torch.from_numpy(gate_pattern['active_values']).to(residual.device)
        active_residuals = residual[active_mask]
        
        # Convert to numpy for storage
        active_residuals_np = active_residuals.detach().cpu().numpy()
        prev_attr_np = prev_attr.detach().cpu().numpy()
        
        return {
                        'active_scaled_residual': scaled_residual[active_mask.nonzero()],
                        'active_indices': active_indices,
                        'prev_attr': prev_attr_np,
                        'mapping': self.mapping,
                        'xyz_before': self.xyz_before,
                        'num_gaussians': current_xyz.shape[0]  # Add this to verify during loading
                    }


    def reconstruct_gated_attribute_residuals(self, compressed_data):
        """
        Reconstructs a gated attribute from its compressed form
        """
        # Create empty tensor of correct shape
        reconstructed = torch.zeros([compressed_data['mapping'].shape[0], 3], device='cuda')
        
        # Get the active mask, gate values, and residual values
        active_indices = torch.tensor(compressed_data['active_indices']).to('cuda')
        # active_gate_values = torch.from_numpy(compressed_data['active_gate_values']).to('cuda')
        # active_gate_values = active_gate_values.squeeze(-1)
        active_scaled_residual = torch.tensor(compressed_data['active_scaled_residual']).to('cuda')
        
        # # Reshape gate values to match residual dimensions
        # # If residuals are 2D (e.g., [N,3] for xyz), we need to add a dimension to gate values
        # if len(active_scaled_residual.shape) > 1:
        #     active_gate_values = active_gate_values.unsqueeze(-1)

        # Apply the gate values to the residuals
        # gated_residuals = active_residuals * active_gate_values
        
        # Fill in the gated residuals
        # reconstructed[active_mask] = gated_residuals
        breakpoint()
        reconstructed[active_indices] = active_scaled_residual.squeeze(1)
        
        return reconstructed # note that you need to add the previous attribute to get the full attribute

    def get_compression_ratio(self, attribute_name):
        """
        Returns the compression ratio for a gated attribute
        """
        compressed = self.compress_gated_attribute(attribute_name)
        original_size = np.prod(compressed['shape'])
        compressed_size = compressed['active_residuals'].size
    ...
    def update_residuals(self):
        ...
        # Initialize parent_idx as a dictionary where each gaussian is its own parent
        self.parent_idx = {i: i for i in range(self._xyz.shape[0])}
        self.mapping =  torch.arange(self._xyz.shape[0], device="cuda")
        ...
        for i, att_name in enumerate(atts):
            if self.latent_args.quant_type[i] == 'sq_res':
                ...
                if self.frame_idx == 2:
                    # Redefine to LatentDecoderRes only if it was DecoderIdentity
                    ...
                    # Do nothing, but still, enforce the decoder to already be LatentDecoderRes
                    # This is for the subsequent frames frame_idx > 2
                    decoder = self.latent_decoders[att_name]
                    assert isinstance(self.latent_decoders[att_name], LatentDecoderRes)

                self.latent_decoders[att_name].frame_idx = self.frame_idx

                # Register the uncompressed latents for the *previous* frame
                ...
                # Register *new* optimizable parameters for the current frame
    ...
    def save_compressed_pkl(self, path, latent_args):
        ...
        # Save the current state of gaussians for comparison
        current_xyz = self.get_xyz.clone()
        ...
        for i,attribute in enumerate(self.param_names):
            if isinstance(self.latent_decoders[attribute], DecoderIdentity):
                if self.prev_atts[attribute] is not None and self.gate_atts is not None and self.gate_params[attribute]:
                    xyz = self.latent_decoders["xyz"](self._latents["xyz"])
                    prev = self.prev_atts["xyz"]
                    residual_xyz = xyz - prev
                    scaled_residual_xyz_without_prev = self.gate_atts.forward_eval(residual_xyz)
                    scaled_residual_xyz_with_prev = self.gate_atts.forward_eval(residual_xyz)+prev
                    ungated_xyz_indices = self.gate_atts.sample_gate(stochastic=False).nonzero(as_tuple=True)[0]

                    reconstructed = self.prev_atts['xyz']
                    reconstructed[ungated_xyz_indices] += scaled_residual_xyz_without_prev[ungated_xyz_indices]
                    assert (scaled_residual_xyz_with_prev != reconstructed).count_nonzero() == 0

                    scaled_residual_xyz_without_prev_nonzero = scaled_residual_xyz_without_prev[ungated_xyz_indices]
                    reconstructed_new = self.prev_atts['xyz']
                    reconstructed_new[ungated_xyz_indices] += scaled_residual_xyz_without_prev_nonzero
                    assert (reconstructed_new != reconstructed).count_nonzero() == 0
                    
                    latents[attribute] = {
                        'ungated_xyz_indices': ungated_xyz_indices.detach().cpu(),
                        'scaled_residual_xyz_without_prev_nonzero': scaled_residual_xyz_without_prev_nonzero.detach().cpu(),
                        'scaled_residual_xyz_without_prev': scaled_residual_xyz_without_prev.detach().cpu(),
                        'mapping': self.mapping,
                        'xyz_before': self.xyz_before,
                        'num_gaussians': current_xyz.shape[0]  # Add this to verify during loading
                    }
        ...
        # Save descriptive text file
        desc_path = os.path.splitext(path)[0] + '_desc.txt'
        with open(desc_path, 'w') as f:
            f.write("=== Save State Description ===\n\n")
            
            # Describe latent decoders
            f.write("Latent Decoders:\n")
            for attr, decoder in self.latent_decoders.items():
                f.write(f"  {attr}:\n")
                f.write(f"    Type: {type(decoder).__name__}\n")
                if hasattr(decoder, 'decoded_att'):
                    f.write(f"    Has decoded_att: True\n")
                    if decoder.decoded_att is not None:
                        f.write(f"    decoded_att shape: {decoder.decoded_att.shape}\n")
                    else:
                        f.write(f"    decoded_att shape: None\n")
                else:
                    f.write(f"    Has decoded_att: False\n")
            
            # Describe latents
            f.write("\nLatents:\n")
            for attr, val in latents.items():
                f.write(f"  {attr}:\n")
                if isinstance(val, dict):  # Gated attributes
                    f.write(f"    Type: gated\n")
                    f.write(f"    scaled_residual_xyz_without_prev_nonzero shape: {val['scaled_residual_xyz_without_prev_nonzero'].shape}\n")
                    f.write(f"    scaled_residual_xyz_without_prev shape: {val['scaled_residual_xyz_without_prev'].shape}\n")
                    f.write(f"    num_gated: {len(val['ungated_xyz_indices'])}\n")
                elif isinstance(val, CompressedLatents):
                    f.write(f"    Type: compressed\n")
                    f.write(f"    Original shape: {val.uncompress().shape}\n")
                else:
                    f.write(f"    Type: {type(val)}\n")
                    if hasattr(val, 'shape'):
                        f.write(f"    Shape: {val.shape}\n")
            
            # Describe decoder state dict
            f.write("\nDecoder State Dict:\n")
            for attr, state in decoder_state_dict.items():
                f.write(f"  {attr}:\n")
                for key, val in state.items():
                    if torch.is_tensor(val):
                        f.write(f"    {key}: shape={val.shape}, dtype={val.dtype}\n")
                    else:
                        f.write(f"    {key}: type={type(val)}\n")
            
            # Describe decoder args
            f.write("\nDecoder Args:\n")
            for attr, args in decoder_args.items():
                f.write(f"  {attr}:\n")
                for key, val in args.items():
                    if torch.is_tensor(val):
                        f.write(f"    {key}: shape={val.shape}, dtype={val.dtype}\n")
                    else:
                        f.write(f"    {key}: type={type(val)}, value={val}\n")
    ...
    def load_compressed_pkl(self, path):
        ...
        for i, attribute in enumerate(latents):
            if self.latent_args.gate_params[i] == 'on':
                # First load the previous attributes if they exist
                if self.prev_atts[attribute] is not None:
                    prev_att = self.prev_atts[attribute]
                else:
                    prev_att = torch.zeros([latents[attribute]["active_mask"].shape[0], 3], device="cuda")
                
                reconstructed_residual = self.reconstruct_gated_attribute_residuals(latents[attribute])
                prev_att_mapping = latents[attribute]["mapping"]
                self.mapping = prev_att_mapping
                
                # Verify mapping is valid
                if prev_att_mapping.max() >= prev_att.shape[0]:
                    raise ValueError(f"Invalid mapping in compressed data: max index {prev_att_mapping.max()} >= prev_att size {prev_att.shape[0]}")
                
                remapped_prev_att = prev_att[prev_att_mapping]
                reconstructed_attribute = reconstructed_residual + remapped_prev_att
                reconstructed_attribute = reconstructed_attribute.cuda()
                
                # Verify the reconstructed attribute has the correct shape
                if reconstructed_attribute.shape[0] != num_gaussians:
                    raise ValueError(f"Reconstructed attribute shape mismatch: {reconstructed_attribute.shape[0]} != {num_gaussians}")
                
                self._latents[attribute] = nn.Parameter(reconstructed_attribute.requires_grad_(False))
            else:
                if self.prev_atts[attribute] is not None:
                    prev_att = self.prev_atts[attribute]
                else:
                ...
                if latent_decoders_dict[attribute] == "LatentDecoder":
                    ...
                    del decoder_state_dict[attribute]['decoded_att']
                    self.latent_decoders[attribute].load_state_dict(decoder_state_dict[attribute])
                    self.latent_decoders[attribute].init_decoded(decoded_att)
                    self.latent_decoders[attribute].identity = False  # Ensure it's not in identity mode
                    self.latent_decoders[attribute].frame_idx = self.frame_idx
                    self._latents[attribute] = nn.Parameter(latents[attribute].uncompress().cuda().requires_grad_(False))
                    # Verify the decoder is working by doing a test decode
                    test_output = self.latent_decoders[attribute](self._latents[attribute])
                    if test_output.shape[-1] != decoder_args[attribute]['feature_dim']:
                        print(f"Warning: {attribute} decoder output shape {test_output.shape} does not match expected feature_dim {decoder_args[attribute]['feature_dim']}")
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, is_dynamic=False):
        ...
        if is_dynamic:
            ...
            self.mapping = torch.cat((self.mapping, torch.nonzero(selected_pts_mask).repeat(N,1).flatten()), dim=0)
        ...
        if is_dynamic:
            ...
            if new_mapping.max() >= self.xyz_before.shape[0]:
                new_mapping[new_mapping >= self.xyz_before.shape[0]] = new_mapping[new_mapping[new_mapping >= self.xyz_before.shape[0]]]
            self.mapping =  self.mapping[~prune_filter]
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, is_dynamic=False):
        ...
        if is_dynamic:
            ...
            if new_mapping.max() >= self.xyz_before.shape[0]:
                new_mapping[new_mapping >= self.xyz_before.shape[0]] = new_mapping[new_mapping[new_mapping >= self.xyz_before.shape[0]]]
            self.mapping = torch.cat((self.mapping, torch.nonzero(selected_pts_mask).flatten()), dim=0)
    ...
    def copy(self):
        ...
        new_model = GaussianModel(self.max_sh_degree, self.latent_args, self.model_args, self.frame_idx)
